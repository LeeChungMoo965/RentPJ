...
<%
    ...
    Member member = modifymember.getMemberById(user_id);
    Member member1 = modifymember.getMemberById("kakao."+user_id);
...
<div class="container py-4">
    ...
    <form action="UpdateMember_process.jsp" method="post">
        ...
        <tr>
            ...
            <%-- 아이디를 출력하면서 서버에도 아이디가 전달되지 않아서
            AI의 도움으로 type을 hidden으로 설정하고 실행하였음 --%>
            <td><%=member1.getId()%> <input type="hidden" name="id" value="<%=member1.getId()%>"></td>
        </tr>

        <tr>
            <th>비밀번호</th>
            <td><input type="password" name="password" value="<%=member1.getPassword()%>"></td>
        </tr>

        <tr>
            <th>이름</th>
            <td><input type="text" name="name" value="<%=member1.getName()%>"></td>
            </tr>
        </tr>

        <tr>
            <th>성별</th>
            <td><%=member1.getGender()%></td>
        </tr>

        <tr>
            <th>생년월일</th>
            <td><%=member1.getBirth()%></td>
        </tr>

        <tr>
            <th>전화번호</th>
            <td><input type="text" name="phone" value="<%=member1.getPhone()%>"></td>
        </tr>

        <tr>
            <th>이메일</th>
            <td><input type="text" name="mail" value="<%=member1.getMail()%>"></td>
        </tr>

        <tr>
            <th>주소</th>
            <td><input type="text" name="address" value="<%=member1.getAddress()%>"></td>
        </tr>

        <tr>
            <th>가입일</th>
            <td><%=member1.getRegist_day()%></td>
        </tr>
    </table>
    <div class="text-center mt-3">
        <button type="submit" class="btn btn-primary">수정하기</button>
