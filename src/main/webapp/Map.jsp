...
var carPositions = [];
<% 
            String day1 = request.getParameter("day1") + "-" + request.getParameter("hour1") + "-" + request.getParameter("min1");
            String day2 = request.getParameter("day2") + "-" + request.getParameter("hour2") + "-" + request.getParameter("min2");
%>
<% 
 
//cars 리스트 유효성 검사
    if(cars != null) {
        //Car 객체 생성 후 cars에있는 Car 객체주소 입력
...
for (var i = 0; i < carPositions.length; i++) {
    ...
    kakao.maps.event.addListener(carMarker, 'click', makeClickListener(map, carMarker, carPositions[i].title, carPositions[i].carprice,carPositions[i].carid, carPositions[i].day1, carPositions[i].day2, carPositions[i].filename));
    
    markerBounds.extend(markerPosition);
    hasValidMarkers = true;
}

function makeClickListener(map, marker, title,carprice,carid,day1,day2,filename) {
    ...
    return function() {
        ...
        infowindow.setContent('<table ><tr><td width="100px" align="left">'+     
            ...
            '<td><a class="btn btn-success" href="Payment.jsp?carid='+carid+'&day1='+day1+'&day2='+day2+'">예약하기</a></tr></table>');
